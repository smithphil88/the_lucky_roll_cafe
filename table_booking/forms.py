...
class BookingForm(forms.ModelForm):
    booking_date = forms.DateField(widget=forms.SelectDateWidget(attrs={
        'class': 'form-select'}), required=True)
    time = forms.ChoiceField(choices=TIME_SLOTS, widget=forms.Select(attrs={
    ...
    table_type = forms.ChoiceField(choices=TABLE_TYPE, widget=forms.Select(
        attrs={'class': 'form-select'}), required=False)
    num_of_guests = forms.IntegerField(
        label="Guests (max-12 people)",
        widget=forms.NumberInput(attrs={
            'class': 'form-control', 'type': 'number', 'min': 1, 'max': 12,
    ...
    class Meta:
        model = Booking
        fields = (
            'table_type', 'booking_date', 'time',
            'num_of_guests', 'additional_message')
        read_only = ['slug']
